...
input=$(cat)
# Skip slash commands and short acknowledgements; fail open (route) if jq is missing or parsing fails.
if command -v jq >/dev/null 2>&1; then
  skip=$(printf '%s' "$input" | jq -r '(.prompt // "") as $p
    | ($p | test("\\A/[A-Za-z0-9_:-]+(\\s|\\z)"))
      or ($p | test("\\A\\s*((ok(ay)?|y(es)?|yep|yeah|sure)[,.!]?\\s*)?(continue|go( on| ahead)?|proceed|do it|next|done|lgtm|looks good|thanks?( you)?|ty|nice|great)?\\s*[.!]*\\s*\\z"; "i"))' 2>/dev/null)
  [ "$skip" = "true" ] && exit 0
fi
